...
export function FollowUpRow({ f, weekOptions, onCycle, onDelete, onEditNote, onMoveWeek }) {
  const [note, setNote] = useState(f.statusNote || '')
  const commit = () => { if (note !== (f.statusNote || '')) onEditNote(f.id, note) }
  ...
  return (
    <TR>
      ...
      <TD className="align-top py-3 w-[140px] text-sm text-foreground/80">
        {f.owner || <span className="text-muted-foreground">—</span>}
      </TD>
      <TD className="align-top py-3 min-w-[220px]">
        <div className="flex items-start gap-2">
          ...
          <input
            ...
            className="flex-1 min-w-0 bg-transparent text-sm text-foreground/80 placeholder:text-muted-foreground/60 focus:outline-none rounded-md px-2 py-1 border border-transparent hover:border-input focus:border-input"
          />
        </div>
      </TD>
      <TD className="align-top py-3 w-[88px]">
